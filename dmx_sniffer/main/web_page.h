...
<div class="row"><label>Кол-во LEDs:</label><input type="number" id="totalLeds" value="1000" min="1" max="4096"><button onclick="sendLedCount()">Применить</button>
</div>
...
<div class="help"><b>WS2812B</b> — 1000 светодиодов на GPIO23 (RMT, 8 МГц). Реверс — меняет порядок пикселей (первый становится последним). Сдвиг — циклический поворот ленты (кольцевой буфер): пиксель из буфера [0] появляется на физическом LED N. Полезно когда физическое начало ленты не совпадает с началом DMX-патча.
</div>
</div>
...
function showStatus(e,t,n){var d=document.getElementById("status"+e);if(!d)return;d.textContent=t,d.className="status "+(n?"ok":"err"),setTimeout(()=>d.textContent="",3e3)}function sendSpeed(e){}function sendLedCount(){var e=parseInt(document.getElementById("totalLeds").value)||1e3;api("POST","/api/leds",{count:e}).then(t=>{t.ok?(document.getElementById("ledPxSlider").max=Math.max(0,e-1),document.getElementById("ledCountSlider").max=e,document.getElementById("ledShiftSlider").max=Math.max(0,e-1),showStatus(0,"LED установлены: "+e,!0),updatePatchInfo(),updateTxSliderMax(),fetchInfoData()):showStatus(0,"Ошибка",!1)})}function sendLedReverse(){var e=document.getElementById("ledReverseSw").checked;api("POST","/api/led_reverse",{reverse:e}).then(t=>showStatus(0,t.ok?"Реверс: "+e:"Ошибка",!!t.ok))}function sendLedShift(){var e=parseInt(document.getElementById("ledShiftSlider").value)||0;document.getElementById("ledShiftVal").textContent=e;api("POST","/api/led_shift",{shift:e})}
